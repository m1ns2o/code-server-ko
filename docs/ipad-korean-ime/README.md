...
```bash
npm run watch -- --bind-addr=0.0.0.0:8080 --auth=none --disable-workspace-trust /path/to/code-server-ko
```

같은 네트워크의 iPad에서 개발 머신의 LAN IP와 포트로 접속합니다.

## Maintenance Notes

- 한글 조합기는 compatibility jamo 범위 `U+3131..U+3163`만 처리합니다.
- 한글 조합/IME 로직은 sibling checkout인 `xterm-ko-ime-adapter`에 있고, code-server patch는 VS Code 터미널에 adapter를 붙이는 얇은 연결부만 유지합니다.
